...
string PostToDoAuthentificator::MakePostParameters(string login, string passwd)
{
...
PostToDoAuthentificator::PostToDoAuthentificator(string username,string  userpasswd)
{
	CURL *curl;
	CURLcode res;
	string response;

	string data = MakePostParameters(username, userpasswd);
	...
	if (curl) {
		...
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, ReplyToString);
		...
		if (CURLE_OK == res) {
			...
			if ((CURLE_OK == res) && ct) {
				...
				if (parsingSuccessful) {
					session_id = parsedFromString["session"].asString();
				}
			}
		}
...
std::string PostToDoAuthentificator::GetSessionID()
{
	return session_id;
}

PostToDoAuthentificator::~PostToDoAuthentificator()
{
}
